 ...
 *
 * <p>
 * Handles the college registration workflow: creates the college entity
 * and provisions the initial admin user account.
 * </p>
 */
@Service
public class CollegeServiceImpl implements CollegeService {
    ...
    public CollegeDto registerCollege(CollegeRegistrationRequest request) {
        if (collegeRepository.findByName(request.getCollegeName()).isPresent()) {
            throw new IllegalArgumentException("College '" + request.getCollegeName() + "' already exists.");
        }
        if (userRepository.existsByEmail(request.getAdminUser().getEmail())) {
            throw new IllegalArgumentException(
                    "User with email '" + request.getAdminUser().getEmail() + "' already exists.");
        }
        ...
        BeanUtils.copyProperties(savedCollege, dto);
        dto.setCollegeId(savedCollege.getCollegeId());
        return dto;
    }
}
